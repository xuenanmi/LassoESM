...
def cv_res(random_seed,Xs, ys, RF_estimators, RF_depth, RF_features, Ada_estimators, Ada_lr, SVC_C, SVC_kernel, MLP_hidden_layers, MLP_batch, MLP_lr):
    """
    Perform cross-validation for multiple classifiers and return their balanced accuracy scores.
    
    Args:
        random_seed (int): Random seed for reproducibility.
        Xs (np.ndarray): Input features.
        ys (list): Target labels.
        RF_estimators (int): Number of estimators for RandomForest.
        RF_depth (int): Maximum depth for RandomForest.
        RF_features (str): Max features for RandomForest.
        Ada_estimators (int): Number of estimators for AdaBoost.
        Ada_lr (float): Learning rate for AdaBoost.
        SVC_C (float): Regularization parameter for SVC.
        SVC_kernel (str): Kernel type for SVC.
        MLP_hidden_layers (tuple): Hidden layer sizes for MLPClassifier.
        MLP_batch (int): Batch size for MLPClassifier.
        MLP_lr (float): Learning rate for MLPClassifier.
    
    Returns:
        list: Cross-validation results (balanced accuracy scores) for each classifier.
    """
...
if __name__ == '__main__': 
   ...
   vanilla_ESM_cv_res, Peptide_ESM_cv_res, rodeo_ESM_cv_res = [], [],[]

   for i in random_seed_ls:
       vanilla_ESM_cv_res.append(cv_res(i, vanilla_Xs, ys, 100, 50, 'sqrt', 200, 0.1, 10, 'linear', (512,64), 16, 0.01))   #utilize the optimized hyperparameters
   ...
   print(result)
   # Process the dataframe and prepare for plotting
